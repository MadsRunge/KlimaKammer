...
class ClimateAnalyzer: #
    ...
    # ERSTAT DENNE FUNKTION MED DEN FORBEDREDE VERSION NEDENFOR
    def _format_bbr_data_for_prompt(self, bbr_data: Optional[BuildingData]) -> str:
        ...
        def add_info(label, value, unit=""):
            # Inkluderer 0, men ikke None, tom streng, eller tom liste
            if value is not None and value != "" and value != []:
                info_parts.append(f"{label}: {value}{unit}")

        add_info("Bygningsnummer (hovedbygning)", bbr_data.building_number)
        add_info("Bygningstype (hovedbygning)", bbr_data.building_type)
        add_info("Opførelsesår (hovedbygning)", bbr_data.building_year)
        if bbr_data.renovation_year and bbr_data.renovation_year != bbr_data.building_year:
            add_info("Renoveret år (hovedbygning)", bbr_data.renovation_year)

        add_info("Ydervægsmateriale (hovedbygning)", bbr_data.exterior_material)
        add_info("Tagdækningsmateriale (hovedbygning)", bbr_data.roof_material)

        add_info("Samlet bygningsareal (hovedbygning)", bbr_data.total_building_area, " m²")
        add_info("Samlet boligareal (hovedbygning)", bbr_data.total_residential_area, " m²")
        add_info("Samlet erhvervsareal (hovedbygning)", bbr_data.total_commercial_area, " m²")
        add_info("Bebygget areal (hovedbygning)", bbr_data.built_area, " m²")

        add_info("Antal etager (hovedbygning)", bbr_data.floors)
        if bbr_data.deviating_floors:
            # Du kan overveje at oversætte 'deviating_floors' koden til noget mere læseligt her
            add_info("Afvigende etager (hovedbygning, f.eks. kælder kode)", bbr_data.deviating_floors)

        # Kælder og loft fra direkte felter, hvis floor_details ikke giver dem
        if not any("Kælder" in part for part in info_parts) and bbr_data.basement_area is not None: 
            add_info("Kælderareal (hovedbygning, direkte felt)", bbr_data.basement_area, " m²")
        
        if not any("Tagetage" in part for part in info_parts) and not any("Loftareal" in part for part in info_parts) and bbr_data.attic_area is not None:
            add_info("Loftsareal (hovedbygning, direkte felt)", bbr_data.attic_area, " m²")

        # Detaljeret kælder- og loftsareal fra floor_details (hvis de findes)
        if bbr_data.floor_details:
            info_parts.append("\nETAGEDETALJER (hovedbygning):")
            for floor in bbr_data.floor_details:
                floor_desc = f"  - Etage '{floor.get('designation', 'Ukendt')}' ({floor.get('type_name', 'Ukendt type')})"
                details = []
                if floor.get('total_area') is not None: details.append(f"Samlet areal: {floor.get('total_area')} m²")
                if floor.get('basement_area') is not None: details.append(f"Kælderareal: {floor.get('basement_area')} m²")
                if floor.get('attic_area') is not None: details.append(f"Loftareal (udnyttet tagetage): {floor.get('attic_area')} m²")
                if floor.get('commercial_area') is not None: details.append(f"Erhverv i kælder: {floor.get('commercial_area')} m²")
                if details:
                    info_parts.append(f"{floor_desc}: {', '.join(details)}")
                else:
                    info_parts.append(floor_desc)
        
        add_info("Varmeinstallation (hovedbygning)", bbr_data.heating_type)
        add_info("Elevator (hovedbygning)", "Ja" if bbr_data.has_elevator else "Nej")
        # add_info("Koordinater (reference)", bbr_data.coordinate) # Typisk ikke nødvendigt for AI'en

        # Tilføj information om yderligere bygninger
        if bbr_data.additional_buildings:
            info_parts.append("\nYDERLIGERE BYGNINGER PÅ GRUNDEN:")
            for i, add_building in enumerate(bbr_data.additional_buildings):
                building_details = []
                if add_building.get('building_number'):
                    building_details.append(f"Nr: {add_building.get('building_number')}")
                if add_building.get('type'):
                    building_details.append(f"Type: {add_building.get('type')}")
                if add_building.get('area') is not None:
                    building_details.append(f"Areal: {add_building.get('area')} m²")
                if add_building.get('year'):
                    building_details.append(f"Opført: {add_building.get('year')}")
                if add_building.get('material'):
                    building_details.append(f"Materiale: {add_building.get('material')}")
                
                if building_details:
                    info_parts.append(f"  Bygning {i+1}: {', '.join(building_details)}")
            info_parts.append("") 

        if not info_parts:
            return "BBR data fundet, men relevante felter er tomme eller mangler for hovedbygningen.\n"
